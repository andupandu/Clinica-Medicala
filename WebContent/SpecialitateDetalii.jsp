...
<img src="resources/medical.png" width="500" height="400" class="center">
  <h3 class="display-4" align="center">Specialitatea <%=spec %></h3>
  <hr class="my-4">
  <section class="container p-t-3">
    <div class="row">
      ...
      <%  if(spec!=null){
      if(spec.equals("Cardiologie")){%>
      Cardiologia este specialitatea care se ocupa cu depistarea, diagnosticarea, tratarea şi recuperarea bolnavilor cu suferinţe cardiovasculare.
      <%}
      else if(spec.equals("Dermatologie")){%>
      Dermato-venerologia este specialitatea care se ocupă cu studiul tuturor patofenomenelor care se produc4
      la nivelul organului cutanat (piele, mucoase accesibile, fanere), la orice vârstă.
      <%}else if(spec.equals("Oftalmologie")){%>
         Oftalmologia se ocupă cu tratarea bolilor organului vederii care include ochiul şi structurile din jurul ochiului, pleoapele şi sistemul lacrimar.
În cadrul cabinetului de oftalmologie se poate face o evaluare oftalmologică completă care constă în examinarea externă a globului ocular,
 testarea acuităţii vizuale, funcţiei pupilare, motilităţii extraoculare, câmpului vizual, presiunii intraoculare şi examinarea structurilor
  oculare prin pupila dilatată (fundul de ochi).
  <%} else if(spec.equals("Stomatologie")){%>
  Stomatologie este ramura medicinii care se ocupa cu prevenirea si tratarea afectiunilor dintilor precum si a celorlalte parti care alcatuiesc, impreuna, cavitatea bucala.
  <%} else if(spec.equals("Neurologie")){%> 
  Neurologia este o ramură specială a medicinii care se ocupă cu diagnosticul și tratamentul bolilor organice care afectează sistemul nervos central sau periferic.
     <%}else if(spec.equals("Pediatrie")){%>
     Pediatria este o ramura a medicinei care se ocupa de bolile copilului, de la nastere si pana la pubertate (inclusiv). Spre deosebire de alte specialitati medicale, pediatria nu se ocupa de o boala (ca de exemplu fiziologia) sau de un aparat sau sistem (ca de exemplu cardiologia), ci de patologia unei perioade foarte intinse (si in multe privinte decisiva) din viata individului.
     <%} } %>
        </div>
        <br>
    </div>
    ...
    <div class="container">
        <h5 class="section-title h1">Medici</h5>
        <div class="row">
            <!-- Team member -->
            <%
            int i=0;
            for(Medic m:DbOperations.getMedicFromCodSpec(DbOperations.getCodSpecFromDenSpec(spec))){%>
                        ...
                        <div class="backside">
                            <div class="card">
                                <div class="card-body text-center mt-4">
                                    ...
                                    <input type="button" value="Vezi detalii" class="btn btn-secondary" onclick="detaliiMedic('<%=spec%>',<%=i%>)">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" id="modal">
        ...
        <form id="consultatie" action="ProgramariConsultatie?pacient=true" method="post" style="display:none;text-align:center"> 
 <table>
